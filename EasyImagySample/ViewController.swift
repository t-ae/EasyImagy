...
class ViewController: UIViewController {
	...
	@IBAction private func onPressActionButton(sender: UIButton) {
		...
		controller.addAction(UIAlertAction(title: "Flip Horizontally", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!).flipX().UIImage
			sender.enabled = true
		})
		
		controller.addAction(UIAlertAction(title: "Flip Vertically", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!).flipY().UIImage
			sender.enabled = true
		})
		
		controller.addAction(UIAlertAction(title: "Rotate clockwise", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!).rotate().UIImage
			sender.enabled = true
		})
		
		controller.addAction(UIAlertAction(title: "Rotate counterclockwise", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!).rotate(-1).UIImage
			sender.enabled = true
		})
		
		controller.addAction(UIAlertAction(title: "Rotate by 180 degrees", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!).rotate(2).UIImage
			sender.enabled = true
		})
		
		controller.addAction(UIAlertAction(title: "Resize", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!).resize(width: 100, height: 100).UIImage
			sender.enabled = true
		})
		
		controller.addAction(UIAlertAction(title: "Crop", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!)[0..<100][0..<100]!.UIImage
			sender.enabled = true
		})
		
		controller.addAction(UIAlertAction(title: "Grayscale", style: .Default) { action in
			imageView.image = Image<Pixel>(UIImage: imageView.image!).map { (pixel: Pixel) -> UInt8 in
				pixel.gray
			}.UIImage
			sender.enabled = true
		})
		...
		controller.addAction(UIAlertAction(title: "Binarize (Auto threshold)", style: .Default) { action in
			...
			imageView.image = image.map { (pixel: Pixel) -> UInt8 in
				pixel.gray < threshold ? 0 : 255
			}.UIImage
			sender.enabled = true
		})
